      ...
      <a-col :md="24" :lg="16">
        ...
        <a-form layout="vertical">
          ...
          <a-form-item
            label="简介"
          >
            <a-textarea rows="4" placeholder="You are not alone." v-model="form.remark"/>
          </a-form-item>
...
import {settings} from '@/api/login'
...
export default {
 ...
 mounted () {
   ...
   this.form.headimg = this.userInfo.avatar
   this.option.img = defaultSettings.baseURL + '/' + this.userInfo.avatar
  },
  methods: {
    setavatar (url) {
      this.option.img = defaultSettings.baseURL + '/' + url
      this.form.headimg = url
    },
    handleSubmit() {
      var _this = this 
      console.log(this.form)
      settings(this.form).then(res => {
        if (res.code == 20000) {
