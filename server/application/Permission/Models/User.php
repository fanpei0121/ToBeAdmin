...
class User
{
    ...
    public function getList($page, $pageSize)
    {
        ...
        $users = Db::table("auth_user")->where("id", "neq", config('permission.auth_super_id'))->select();
        foreach ($users as &$user) {
            $user["roles"] = Db::table("auth_user_role_access")->where("user_id", $user["id"])->column("role_id");
            $user["rules"] = Db::table("auth_role_permission_access")->where("role_id", "in", $user["roles"])->column("permission_id");
        }
